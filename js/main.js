...
/**
 * video-export.js
 * Builds a real time-lapse video entirely in the browser: draws each saved
 * photo to a canvas in sequence, captures the canvas as a MediaStream, and
 * records it with MediaRecorder. No server involved — output is a .webm
 * file the user can download.
 */
const VideoExport = (() => {
  const FRAME_MS = 550; // how long each photo is shown
  const WIDTH = 720;
  const HEIGHT = 960;
  ...
  async function generate(entries, onProgress) {
    ...
    if (typeof MediaRecorder === 'undefined') {
      throw new Error('This browser does not support video recording (MediaRecorder unavailable).');
    }
    ...
    const canvasStream = canvas.captureStream(2); // 2 fps base; we redraw manually
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      drawLabel(ctx, `Day ${entry.dayNumber}`);
  ...
  function drawLabel(ctx, text) {
    ...
    ctx.fillRect(0, HEIGHT - 70, metrics.width + padding * 2, 70);
    ctx.fillStyle = '#8FD13F';
    ctx.fillText(text, padding, HEIGHT - 20);
  }
